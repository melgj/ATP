...
sr_current <- sr_df %>% 
  filter(Lag < 15, Rating >= 1600, Games >= 30)

view(head(sr_current,25))

### Write ratings to csv file
write_csv(sr_current, "atp_Clay_SR_ratings_jan21.csv")
...
ggplot(temp,
  ...
  labs(title = "Clay Court ATP Player Ratings Through Time - Current Top 20 Rated Players (Stephenson ELO Variant)", 
       x = "Time", y = "Rating") +
  scale_x_discrete(labels = NULL) +
  ylim(1500,2100) +
  geom_hline(yintercept = c(1600, 1800, 2000), col = "grey", lty = 2) +
  geom_line() +
